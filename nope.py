...
class NopeBot(Plugin):
    ...
    async def nope(self, evt: MessageEvent) -> None:
        ...
        if bot_message_id is None:
            ...
            await evt.respond(
                "> **Usage:**  \n"
                "> Reply to the bot's message that you want to remove with `!nope` or react to it"
                " with ❌ emoji.  \n"
                "> It only works with messages that are direct reply to your own command"
                " or reply to a command in general if you have permission to delete messages of"
                " other users in this room."
            )
            return
        bot_message: MessageEvent = await self.client.get_event(
            room_id=evt.room_id,
    ...
    async def try_redact(self, evt: BaseRoomEvent, bot_message: MessageEvent) -> None:
        ...
        try:
            ...
            create_event = next(
                (s_evt for s_evt in state_events if s_evt.type == EventType.ROOM_CREATE),
                None
            )
        ...
        if (
                user_message.sender == evt.sender
                or user_level
                and power_levels
                and user_level >= power_levels.redact
        ):
            await self.client.redact(
                room_id=evt.room_id,
                event_id=bot_message.event_id,
                reason=f"Message redacted with !nope by {evt.sender}"
            )
